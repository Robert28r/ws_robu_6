...
class ObastacleAvoidanceSimple(Node):
    ...
    def obstacle_avoidance(self):
        ...
        for i in range(1,segment_size):
            begin = end
            end = begin + segment_distance_size + 1 #TODO: Plus für Mang!!
        ...
        if segment == 0:
            ang_vel = 0.0 #rad/s
        elif (segment == segment_order[-1]) and len(segment_distances[segment]) == 0: #Überall sind Hindernisse
            ang_vel = 0.0
            segment = -1 # kein freies Segment vorahnden
        elif (segment <= segment_size/2): #Wir drehen uns nach rechts
